...
const StudentList = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setOpen(false);
      setProfile(null);
      setStudentInfo(true); // Show success info modal if needed
    } catch (error) {
                ...
                users
                  ...
                  .map((student, idx) => (
                    <tr key={student.id || idx} className="border-b hover:bg-gray-50">
                      ...
                      <td className="p-3 text-purple-700 font-semibold">{student.admissionNumber}</td>
                      ...
                      <td className="p-3">
                        {users.find(user => user.id === student.parentId)?.name || 'N/A'}
                      </td>

                      <td className="p-3 text-gray-700 truncate max-w-xs">{student.email}</td>
                        ...
                        {openDropdown === student.id && (
                          <div className="absolute right-12 mt-2 w-42 bg-white border rounded shadow z-10">
                            <button
                              ...
                              onClick={() => {
                                // Implement Add Parent details functionality here
                                alert('Add Parent feature to implement');
          ...
          <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            <div className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-lg font-semibold mx-auto">Add New Student</h3>
                  <span onClick={() => setOpen(false)} className="text-red-600 cursor-pointer">×</span>
                </div>
